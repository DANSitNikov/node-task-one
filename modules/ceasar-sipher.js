function caesarCipher(string, shift, operation) {
  const lettersArray = string.split('');
  const alphabet = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];
  const numberSign = operation === 'encode' ? shift : -shift;
  ...
  return lettersArray.map((letter) => {
    ...
    if (alphabet.includes(letterInLowerCase)) {
      let index = (alphabet.indexOf(letterInLowerCase) + numberSign) % alphabet.length;
